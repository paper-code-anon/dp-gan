...
path = f'accel_dp_gan_{starting_time}'
try:
    os.stat(path)
except:
    os.mkdir(path)
# Copy a timestamped version of this file into the new directory
copy(__file__, path + f'/accel_dp_gan_{starting_time}.py')

for iteration in range(1):
    torch.manual_seed(iteration)

    for epsilon in [5]:  #[10, 8, 6, 4]:
        # Matching inverse transformation epsilon to original epsilon
        # inverse_transform_eps = epsilon
        inverse_transform_eps = 100000
    #     epsilon = 100000
        delta = 0.001
        ...
        df = pd.read_csv('accel_x_y_z+ppgv3_removed_nulls_balanced.csv').drop(columns=['subject'])
        ...
        # weights = labels_weights[activity_label]
        ...
        for epoch in range(num_epochs):
            ...
            if (epoch % plot_increment == 0 and epoch != 0) or epoch == num_epochs - 1:
                with torch.no_grad():
                    ...
                    #              label='fake', color='b')
                    #     closest_original_index = np.argmin(list(np.linalg.norm(fake_sample.squeeze() - original) for original in X_unscaled))
                    ...
                    # f, axes_array = plt.subplots(len(random_X), 1, figsize=(15, 5 * num_original_samples))  # Hardcoded size but limitation of max dimension
                    # plt.subplots_adjust(hspace=0.4)
                    # for original, axis in zip(random_X, axes_array):
                    #     axis.plot(range(len(original)), original, label='original', color='orange')
                    #     closest_fake_index = np.argmin(list(np.linalg.norm(fake_sample.squeeze() - original) for fake_sample in fake_samples))
                    ...
                    pd.DataFrame(fake_samples_saving).to_csv(path + f'/generated_fakes_normalised_{iteration}.csv', header=df.columns, index=False)
                    if applying_scaling:
                        ...
                        reverse_transformed_fakes = np.concatenate([
                            np.apply_along_axis(
                                ...
                                axis=1,
                                arr=reverse_transformed_fakes[:, 0:100]),
                            np.apply_along_axis(
                                lambda m: moving_average(m, rolling_mean_window),
                                axis=1,
                                arr=reverse_transformed_fakes[:, 100:200]),
                            np.apply_along_axis(
                                lambda m: moving_average(m, rolling_mean_window),
                                axis=1,
                                arr=reverse_transformed_fakes[:, 200:300]),
                            np.apply_along_axis(
                                lambda m: moving_average(m, rolling_mean_window),
                                axis=1,
                                arr=reverse_transformed_fakes[:, 300:400])],
                            axis=1
                        )
                        pd.DataFrame(reverse_transformed_fakes).to_csv(path + f'/generated_fakes_{iteration}.csv', header=df.columns, index=False)
